...
(ns automaton-optimization.randomness.impl.distribution.factory-test
  (:require
   ...
   [automaton-optimization.randomness.impl.distribution.registry :as
                                                                 opt-registry]
...
(deftest build-test
  (testing "Are builder working for all variants"
    (is (boolean? (-> (opt-registry/registry)
                      (sut/build :bernoulli (prng) {:p 0.3})
                      opt-proba-distribution/draw)))
    (is (float? (-> (opt-registry/registry)
                    (sut/build :beta
                               (prng)
                               {:beta 0.5
                                :alpha 0.7})
                    opt-proba-distribution/draw)))
    (is (integer? (-> (opt-registry/registry)
    ...
    (is (integer? (-> (opt-registry/registry)
                      (sut/build :binomial
                                 ...
                                 {:n 5
                                  :p 0.7})
                      opt-proba-distribution/draw)))
    (is (keyword? (-> (opt-registry/registry)
