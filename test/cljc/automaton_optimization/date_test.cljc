(ns automaton-optimization.date-test
  ...
  (:require [automaton-optimization.date :as sut]
...
(defn validate-date-protocol-constraints
  ...
  * `random-delay-builder`"
  ...
  (let [date-stub (random-date-builder)
        date-test-registry (merge (core-protocols-binary-relation/equivalence #'sut/equal-date
                                                                              (repeatedly 10 random-date-builder)
                                                                              30)
                                  (core-protocols-binary-relation/total-order #'sut/compare-date
                                                                              #'sut/equal-date
                                                                              (repeatedly 10 random-date-builder)
                                                                              30)
                                  {::add-delay-creates-new-date {:msg "A random date, with a positive delay creates a new date with different time"
                                                                 :result (-> date-stub
                                                                             (sut/add-delay (random-delay-builder)))
                                                                 :expect-fn #(and (some? %)
                                                                                  (not (sut/equal-date % date-stub)))}
                                   ::substract-delay-creates-new-date {:msg "A random date, with a positive delay creates a new date with different time"
                                                                       :result (-> date-stub
