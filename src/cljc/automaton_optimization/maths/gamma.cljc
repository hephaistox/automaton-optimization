(ns automaton-optimization.maths.gamma
  "Calculation of gamma function"
  (:refer-clojure :exclude [abs])
  (:require
   [automaton-optimization.maths :refer [HALF_LOG_2_PI
                                         LANCZOS_G
                                         PI
                                         SQRT_2_PI
                                         abs
...
(def ^:no-doc A [0.611609510448141581788E-08 0.624730830116465516210E-08])

(def ^:no-doc B
  [0.195755836614639731882E-09
...
(def ^:no-doc CA -0.422784335098467139393487909917598E+00)

(def ^:no-doc CB 0.577215664901532860606512090082402E+00)
...
(defn lower-regularized-gamma
  ...
  [a x]
  (when (and (>= x 0) (> a 0))
    (let [max-iter (-> (+ (* (log (if (>= a 1) a (/ 1 a))) 8.5) (* 0.4 a) 17)
                       inc
                       floor
                       int)]
      (if (< x (inc a))
  ...
  (let [fpmin 1e-30
    ...
    (loop [m 1
      ...
      (let [m2 (* 2 m)
            ...
            del (* d c)
            h (* h del)]
        (if (or (< (abs del) 3e-7) (>= m 100)) h (recur (inc m) h c d))))))

(defn ibeta
  "Returns the incomplete beta function I_x(a,b)."
  [x a b]
  (when (<= 0 x 1)
    (let [bt (if (or (== x 0) (== x 1))
               0
               (exp (+ (- (log-gamma (+ a b)) (log-gamma a) (log-gamma b))
                       (* a (log x))
                       (* b (log (- 1 x))))))]
      (if (< x (/ (inc a) (+ a b 2)))
        (* bt (/ (betacf x a b) a))
...
(defn erf
  ...
  (let [x' (abs x)
        t (/ 2 (+ x' 2))
        ty (- (* 4 t) 2)
        [d dd] (reduce (fn [[d dd] cof] (vector (+ (- (* ty d) dd) cof) d))
                       [0 0]
                       [-2.8E-17
                        1.21E-16
                        -9.4E-17
    ...
    (if (neg? x) (- res 1) (- 1 res))))

(defn erfc "Computes the complementary error function." [x] (- 1 (erf x)))
