(ns automaton-optimization.randomness.impl.prng.well
  ...
  #?(:cljs (:require-macros
     ...
     :clj (:require
           [automaton-optimization.randomness.impl.prng.well-macros :refer
                                                                    [add-mod-32
                                                                     mat0-neg
                                                                     mat0-pos
                                                                     unsign]]))
...
(defn well-rng-1024a
  ...
  ([]
   (well-rng-1024a
    (long-array 32 (repeatedly 32 #(rand-int opt-maths/infinity-integer)))
    (rand-int 32)))
   ...
   (let [m1 3
     ...
     (aset state
           ...
           (bit-xor (bit-xor (mat0-neg -11 z0) (mat0-neg -7 z1))
                    (mat0-neg -13 z2)))
     (lazy-seq (cons (unsign (* (aget state new-index) fact))
