(ns automaton-optimization.randomness.impl.distribution.registry
  ...
  (:require
   [automaton-optimization.randomness.impl.distribution.exponential :as
                                                                    opt-expo]
   [automaton-optimization.randomness.impl.distribution.kixi-stats
    :as opt-kixi-stats]
...
(defn registry
  ...
  {:bernoulli (fn [_prng {:keys [p]}] (opt-kixi-stats/make-bernoulli p))
   ...
   :gamma (fn [_prng {:keys [shape scale]}]
            (opt-kixi-stats/make-gamma-scale shape scale))
   :log-normal (fn [_prng {:keys [location scale]}]
                 (opt-kixi-stats/make-log-normal location scale))
   :multinomial (fn [_prng {:keys [n probs]}]
                  (opt-kixi-stats/make-multinomial n probs))
   :normal (fn [_prng {:keys [location scale]}]
             (opt-kixi-stats/make-normal location scale))
   :pareto (fn [_prng {:keys [scale shape]}]
   ...
   :weibull (fn [_prng {:keys [shape scale]}]
